...
proc = trtxt.TextProcessor('sp')

preprocessed_data = prtxt.init_sp()
...
pickle.dump(classifiers, open('model/model_hate_sp.pkl', 'wb'))
...
for i in range(len(metrics)):
    ...
    for chave, valor in results_tests_metrics.items():
        res_mets.append(valor['sp_test_' + metrics[i]])
